...
namespace AlgorithmicTrader
{
    ...
    internal class Strategy
    {
        ...
        public void Trade(string[] authorizedData, string symbol)
        {
            ...
            while (true)
            {
                ...
                // Look for trend.
                trendSignal = VolumeSpreadAnalysis(symbol, symbolId);
        ...
        public string VolumeSpreadAnalysis(string symbol, int symbolId)
        {
            double[,] candleSticks;
            ...
            while (true)
            {
                ...
                if (candleSticks != null)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        Console.WriteLine("Array " + symbolId + ": " + i);
                        for (int j = 0; j < 4; j++)
                        {
                ...
                if (Double.IsNaN(0 / candleSticks[9, 3]))
                {
                    // Check if the last candle closed higher then the second one before it.
                    if (candleSticks[9, 2] > candleSticks[7, 2])
                    {
                        // If volume is lower than the two bars before it.
                        if (candleSticks[9, 3] < candleSticks[8, 3] && candleSticks[9,3] < candleSticks[7,3])
                        {
                            Console.WriteLine(symbolId + " Sell signal!");
                            return "Sell";
                        }
                        // If volume is higher than the two bars before it.
                        else if (candleSticks[9, 3] > candleSticks[8, 3] && candleSticks[9, 3] > candleSticks[7, 3])
                        {
                            Console.WriteLine(symbolId + " Buy signal!");
                            return "Buy";
                        }
                    }
                    // Check if the last candle closed lower than the second one before it.
                    else if (candleSticks[9, 2] < candleSticks[7, 2])
                    {
                        // if volume is lower than the two bars before it.
                        if (candleSticks[9, 3] < candleSticks[8, 3] && candleSticks[9, 3] < candleSticks[7, 3])
                        {
    ...
    internal static class Request
    {
        ...
        internal static double[,] candlestickData(string accessToken, string apiServer, int symbolId)
        {
            ...
            // five minute data.
            int startMin = ((int)Math.Floor(((double)(DateTime.Now.Minute - 85) / 5)) * 5);
            // one minute data.
