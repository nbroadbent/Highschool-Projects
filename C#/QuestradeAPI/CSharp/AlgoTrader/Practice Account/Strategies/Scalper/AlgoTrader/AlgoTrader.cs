    ...
    {       
        ...
        public string MovingAverages(string symbol, int symbolId)
        {
            ...
            while (true)
            {
                ...
                // Get 2 min candlestick data for past 20 candles.
                Thread.Sleep(500);
                ...
                if (candleSticks != null)
                {
                    ...
                    if (empty == 0)
                    {
                        ...
                        {
                            if (quantity > 0)                           
                                percentChange = Math.Round(((price / fillPrice) * 100 - 100), 2);
                            else
                                percentChange = Math.Round(((fillPrice / price) * 100 - 100), 2);

                            // Get stop loss and profit goal.
                            if (stopFlag)
                            {
                                // IF THE FILL PRICE IS THE SAME AS THE STOP PRICE, THEN MOVE THE STOP PRICE BY ONE CENT
                                if (quantity > 0)
                                {
                                    Store.stopLoss(fillPrice, candleSticks[9, 0]);
                                    stopPrice = candleSticks[9, 0];
                                    profitGoal = ((fillPrice / stopPrice) * 100 - 100) * 2;

                                    if (fillPrice == stopPrice)
                                    {
                                        stopPrice -= 0.01;
                                    }
                                    //Order.Stop(accessToken, apiServer, symbol, "Sell", accountNumber, quantity, candleSticks[9, 0]);
                                }
                                else if (quantity < 0)
                                {
                                    Store.stopLoss(fillPrice, candleSticks[9, 1]);
                                    stopPrice = candleSticks[9, 1];
                                    profitGoal = ((stopPrice / fillPrice) * 100 - 100) * 2;

                                    if (fillPrice == stopPrice)
                                    {
                                        stopPrice += 0.01;
                                    }
                                    //Order.Stop(accessToken, apiServer, symbol, "Buy", accountNumber, quantity, candleSticks[9, 1]);
                                }
                                profitGoal = Math.Round(profitGoal, 2);
                                stopFlag = false;
                            }   
                            ...
                            Console.WriteLine("stop price: " + stopPrice);
                            Console.WriteLine("proft goal: " + profitGoal);
                        ...
                        wmaLong = Math.Round(Calculate.WeightedMovingAverage(candleSticks, 8), 2);
                        wmaShort = Math.Round(Calculate.WeightedMovingAverage(candleSticks, 5), 2);
                        stochastic = Calculate.Stochastic(candleSticks, 7);
                        // wmaLong = Calculate.WeightedMovingAverage(candleSticks, 8);
                        ...
                        if (wmaShort == wmaLong)
                        {
                            ...
                            while (oldWmaShort == 0 || oldWmaShort == wmaShort)
                            {
                                oldWmaShort = Math.Round(Calculate.WeightedMovingAverage(candleSticks, 5, j), 2);
                                j++;
                            }
                        }

                        Console.WriteLine("wmaLong: " + wmaLong);
                        Console.WriteLine("wmaShort: " + wmaShort);
                        Console.WriteLine("oldWmaShort: " + oldWmaShort);

                        Console.WriteLine();
                        Console.WriteLine("Fast k: " + stochastic[0]);
                        Console.WriteLine("Fast d: " + stochastic[1]);
